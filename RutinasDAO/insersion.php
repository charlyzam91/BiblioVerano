<?php 
	...
	function Insert_Ficha($SiuS, $GRAD, $ESC, $HOR){
		$sql="INSERT INTO `ficha`(`id_ficha`, `id_usuario`, `id_grado`, `id_escuela`, `id_horario`) VALUES (NULL,'$SiuS','$GRAD','$ESC','$HOR')";
		conectar();
		$res=mysql_query($sql);
		desconectar();	
		if ($res) {
	...
	function inserta_plan($pers,$semana, $grado, $turno){
		$sql="INSERT INTO `planeacion`(`id_plan`, `id_persona`, `id_semana`, `id_grado`, `id_turno`) VALUES (NULL,'$pers','$semana','$grado','$turno')";	
	conectar();
		$res=mysql_query($sql);
		desconectar();	
		if ($res) {
