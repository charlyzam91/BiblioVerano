<?php 
	...
	function Rev_GradoP(){
		$sql="SELECT count(id_grado) as fichas FROM `ficha` where id_grado= 1 and estatus =1";
		conectar();
		$res=mysql_query($sql);
		desconectar();
		$fila=mysql_fetch_assoc($res);
		...
		$sql="SELECT count(id_grado)as fichas FROM `ficha` where id_grado= 2 or id_grado=3 and estatus =1";
		conectar();
		$res=mysql_query($sql);
		desconectar();
		$fila=mysql_fetch_assoc($res);
		...
		$sql="SELECT count(id_grado)as fichas FROM `ficha` where id_grado= 4 or id_grado=5 and estatus =1";
		conectar();
		$res=mysql_query($sql);
		desconectar();
		$fila=mysql_fetch_assoc($res);
		...
		$sql="SELECT count(id_grado)as fichas FROM `ficha` where id_grado= 6 or id_grado=7 and estatus =1";
		conectar();
		$res=mysql_query($sql);
		desconectar();
		$fila=mysql_fetch_assoc($res);
	...
	function Revisa_Planeacion($pers,$semana, $grado, $turno){
		$sql="SELECT count(id_persona) as plan_hecho FROM `planeacion` WHERE  `id_semana`='$semana' and `id_grado`='$grado' and `id_turno`='$turno' and estatus = 1";
		conectar();
		$res=mysql_query($sql);
		desconectar();
		$fila=mysql_fetch_assoc($res);
	...
	function Revisa_Planeacion_Personal($per){
		$sql="SELECT count(id_persona) as plan_hecho FROM `planeacion` WHERE  `id_persona`='$per' and estatus = 1";
		conectar();
		$res=mysql_query($sql);
		desconectar();
		$fila=mysql_fetch_assoc($res);
	...
	function revisarFicha($usuario){
		 $sql="SELECT count(*) as fichaactiva FROM `ficha` WHERE `id_usuario` = '$usuario' and `Estatus` = 1";
		conectar();
		$res=mysql_query($sql);
		desconectar();
		$fila=mysql_fetch_assoc($res);
			if ($fila['fichaactiva']>0){	
		return 1 ;
		}else{
		return 0 ;
		}
